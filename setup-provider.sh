...
if ! command -v "unzip" &> /dev/null; then
  echo "Missing required tool unzip. This tool should come by default on Macs, and can be installed via apt on linux"
  exit 1
fi
...
# Rename the extracted file to the generic provider name. Note the required 'v' in front of VERSION_TAG
